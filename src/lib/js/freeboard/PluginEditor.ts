...
export class PluginEditor {
  constructor(jsEditor: JSEditor, valueEditor: ValueEditor) {
        ...
        _.each(settingsDefs, function (settingDef) {
          ...
          switch (settingDef.type) {
            case 'array': {
                    ...
                    $('<ul class="board-toolbar"></ul>').append(
                      $('<li></li>').append(
                        $('<i class="icon-trash icon-white"></i>').click(
                          function () {
                            var subSettingIndex = newSettings.settings[
                              settingDef.name
                            ].indexOf(newSetting);
              ...
              _.each(
                currentSubSettingValues,
                function (currentSubSettingValue, subSettingIndex) {
      ...
      new DialogBox(form, title, 'Save', 'Cancel', function () {
        ...
        for (var index = 0; index < selectedType.settings.length; index++) {
          ...
          if (
            ...
            !_isNumerical(newSettings.settings[settingDef.name])
          ) {
