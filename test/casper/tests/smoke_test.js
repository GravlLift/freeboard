...
casper.test.begin('Freeboard smoke test', function testFunction(test) {
  ...
  casper.then(function () {
    ...
    this.on('click', util.SELECTORS.addPane);
    test.assertVisible(util.SELECTORS.allPanes);

    // Click add widget
    this.on('click', util.SELECTORS.addWidget);
    test.assertVisible(util.SELECTORS.pluginTypeDropdown);

    // Select text widget
    util.setValueAndTriggerChange(
      test,
      util.SELECTORS.pluginTypeDropdown,
      'text_widget'
    );
    ...
    this.on('click', util.SELECTORS.autofillDatasource);
  ...
  casper.then(function () {
    ...
    this.on('click', util.SELECTORS.trashWidget);
  });
  ...
  casper.then(function () {
    ...
    this.on('click', util.SELECTORS.trashPane);
  });
  ...
  casper.then(function () {
    ...
    this.on('click', util.SELECTORS.trashDatasource);
  });
