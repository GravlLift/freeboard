...
export class PluginEditor {
  constructor(private jsEditor: JSEditor, private valueEditor: ValueEditor) {}
  private _displayValidationError(settingName: string, errorMessage: string) {
    var errorElement = $('<div class="validation-error"></div>').html(
      errorMessage
    );
    $('#setting-value-container-' + settingName).append(errorElement);
  }

  private _removeSettingsRows() {
  ...
  public createPluginEditor(
      ...
      _.each(settingsDefs, (settingDef) => {
        ...
        switch (settingDef.type) {
          case 'array': {
            ...
            var currentSubSettingValues: { [key: string]: string }[] = [];

            // Create our headers
            _.each(settingDef.settings, (subSettingDef: Settings) => {
              ...
              subsettingRow.append(
                $('<td class="table-row-operation"></td>').append(
                  $('<ul class="board-toolbar"></ul>').append(
                    $('<li></li>').append(
                      $('<i class="icon-trash icon-white"></i>').on(
                        'click',
                        () => {
                          var subSettingIndex =
              ...
              if (typeaheadSource && settingDef.typeahead_field) {
                ...
                input.on('change', (event) => {
                  ...
                  $.get(source, (data) => {
                    ...
                    data = _.select(data, (elm) => {
                      return elm[settingDef.typeahead_field][0] == value[0];
                    });

                    typeaheadValues = _.map(data, (elm) => {
    ...
    var typeSelect: JQuery<HTMLSelectElement>;

    if (pluginTypeNames.length > 1) {
      var typeRow = createSettingRow('plugin-types', 'Type');
      typeSelect = $<HTMLSelectElement>('<select></select>').appendTo(
      ...
      typeSelect.on('change', () => {
        newSettings.type = $(this).val() as SettingsType;
        newSettings.settings = {};

        // Remove all the previous settings
        this._removeSettingsRows();

        selectedType = pluginTypes[typeSelect.val() as string | number];
